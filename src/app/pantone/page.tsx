...
const processPantoneColors = (data: any): UnifiedPantoneColor[] => {
  ...
  return data.colors.map((color: any) => {
    ...
    return {
      ...
      hex: color.hex || "",
      rgb: color.rgb
        ? ([color.rgb.r || 0, color.rgb.g || 0, color.rgb.b || 0] as [
            number,
            number,
            number
          ])
...
const categorizeColor = (name: string): string => {
  ...
  if (
    ...
    lowerName.includes("apricot") ||
    lowerName.includes("coral") ||
    ...
    lowerName.includes("bright") ||
    lowerName.includes("fluorescent")
  ) {
    return "霓虹色";
  }
...
const categorizeColorType = (name: string): "warm" | "cool" | "neutral" => {
  ...
  if (warmKeywords.some((keyword) => lowerName.includes(keyword)))
    return "warm";
  if (coolKeywords.some((keyword) => lowerName.includes(keyword)))
    return "cool";
  return "neutral";
};
...
const ColorCard: React.FC<ColorCardProps> = ({
  ...
  const textColor = ["#FFFFFF", "#F7F7F7", "#FFFAFA", "#FEFEFE"].includes(
    hexWithHash.toUpperCase()
  )
    ? "#000"
    : "#FFF";
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                {color.lab && (
                  <p className="text-xs text-gray-500">
                    LAB({color.lab.l.toFixed(1)}, {color.lab.a.toFixed(1)},{" "}
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between">
            <div className="text-xs text-gray-500">
              ...
              {color.lab ? (
                <div>
                  LAB({color.lab.l.toFixed(0)}, {color.lab.a.toFixed(0)},{" "}
                  {color.lab.b.toFixed(0)})
                </div>
              ) : color.cmyk ? (
...
export default function PantoneColorPage() {
  ...
  const categories = useMemo(() => {
    const allCategories = Array.from(
      new Set(allPantoneColors.map((color) => color.category))
    );
  ...
  // 统计信息
  const stats = useMemo(() => {
    const newColorsCount = allPantoneColors.filter(
      (c) => c.source === "new-175"
    ).length;
    const classicColorsCount = allPantoneColors.filter(
      (c) => c.source === "panton-2799"
    ).length;
    return {
      newColorsCount,
      classicColorsCount,
      total: allPantoneColors.length,
    };
  }, []);
  ...
  const exportColors = () => {
    ...
    a.href = url;
    a.download = `pantone-colors-${selectedCategory}-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
    a.click();
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  ...
                  <span className="ml-2 text-green-600">
                    新175色: {stats.newColorsCount}
                  </span>
                  <span className="ml-2 text-blue-600">
                    经典系列: {stats.classicColorsCount}
                  </span>
                </p>
              </div>
            </div>
